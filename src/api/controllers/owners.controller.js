const { createUser, getUserWithSocialIdAndEmail, updateSocialLoginId, getUserByEmailPassword, getUserByEmail, updateOtp, findUserWithOtp, updatePassword } = require('../reositories/owners.repository');
...
module.exports = {
    signupWithEmail: async (req, res) => {
        try {
            ...
            const token = getJwtToken(user._id, "Owner");
            user = JSON.parse(JSON.stringify(user))
            delete user.password;
            return __SSR(res, "Signup successfully", { user, token });
        } catch (error) {
            return __SFR(res, "Email/Phone already exist.")
        }
    },
    ...
    socialSignUp: async (req, res) => {
        try {
            ...
            return __SFR(res, "An error occured", error)
        }
    },
